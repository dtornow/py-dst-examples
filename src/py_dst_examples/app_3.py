...
def rollback(ctx, ikey):
    ...
    transfers = conn.execute("SELECT account_id, amount FROM transfer WHERE transfer_id = ?", (ikey,)).fetchall()
    for account_id, amount in transfers:
        # Revert the changes in balance
        conn.execute("UPDATE accounts SET balance = balance - ?, version = version + 1 WHERE account_id = ?", (amount, account_id))
...
def get_balance(ctx, account):
    conn = ctx.deps.get("conn")
    # TRY THIS NEXT
    # resp = conn.execute("""SELECT balance - IFNULL((SELECT SUM(amount) FROM transfers WHERE account_id = ? AND amount > 0), 0), version FROM accounts WHERE account_id = ?""", (account, account)).fetchone()
...
def set_balance(ctx, id, account, amount, version=None):
    conn = ctx.deps.get("conn")
    conn.execute("INSERT INTO transfer (transfer_id, account_id, amount) VALUES (?, ?, ?)", (id, account, amount))
    if version is None:
